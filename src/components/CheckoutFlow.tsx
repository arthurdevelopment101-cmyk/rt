...
export default function CheckoutFlow({
  ...
  const pointsCalculation = React.useMemo(() => {
    ...
    const cashDiscount = total * (discountPercentage / 100);
  ...
  const handlePaymentSubmit = async (e: React.FormEvent) => {
    ...
    // Points from spending (1 Point per 10 EGP * multiplier)
    const basePts = Math.round((finalPayable / 10) * multiplier);
                        ...
                        <div className="flex items-start gap-2.5">
                          ...
                          <label htmlFor="use-loyalty-points" className="cursor-pointer select-none">
                            ...
                            <span className="block text-[10px] text-brand-outline font-light mt-1 leading-relaxed">
                              استخدم <span className="font-bold text-brand-gold font-mono">{pointsCalculation.pointsToRedeem} نقطة</span> للحصول على خصم <span className="font-bold text-brand-umber font-mono font-bold">{pointsCalculation.discountPercentage}%</span> بقيمة <span className="font-bold text-brand-umber font-mono font-bold">EGP {pointsCalculation.cashDiscount.toLocaleString()}</span>.
                            </span>
                          </label>
                        </div>
