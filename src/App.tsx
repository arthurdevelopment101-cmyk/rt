...
import { Product, CartItem, UserProfile, getTierFromSpent, Order, PromoCode, Reward } from "./types";
...
export default function App() {
  ...
  }, [fetchProducts, fetchOrders, fetchPromos, fetchRewards]);
  ...
  const [detailSelectedMaterial, setDetailSelectedMaterial] = React.useState<string>("");
  const [detailSelectedSize, setDetailSelectedSize] = React.useState<string>("");
  ...
  React.useEffect(() => {
    if (selectedProduct) {
      ...
      setDetailSelectedMaterial(selectedProduct.materialOptions?.[0] || "");
      setDetailSelectedSize(selectedProduct.sizeOptions?.[0] || "");
    }
  }, [selectedProduct]);
  ...
  const matchedPromo = promos.find((p) => p.code === activePromo && p.isActive);
  const discountMultiplier = matchedPromo ? matchedPromo.discountPercentage / 100 : 0;
  ...
  const handleApplyPromo = (e: React.FormEvent) => {
    ...
    if (code === "") {
      setPromoError("Please enter a valid code / الرجاء إدخال كود صالح.");
      return;
    }
    ...
    if (found) {
      if (found.isActive) {
        ...
        setPromoSuccess(`${found.code} applied! Enjoy ${found.discountPercentage}% discount / تم تطبيق كود الخصم! استمتع بخصم ${found.discountPercentage}%.`);
      } else {
        setPromoError("This promo code is currently inactive / هذا الكوبون غير فعال حالياً.");
      }
    } else {
      setPromoError("Promo code not recognized / كود الخصم غير صحيح.");
    }
    setPromoInput("");
  };
                ...
                <div className="lg:col-span-5 flex flex-col justify-between">
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="flex gap-4">
                          {selectedProduct.materialOptions.map((hex, i) => {
                            const isSelected = detailSelectedMaterial === hex;
                            return (
                              <button
                                key={i}
                                onClick={() => setDetailSelectedMaterial(hex)}
                                className="w-10 h-10 rounded-full border-2 transition-all relative flex items-center justify-center shadow-sm cursor-pointer"
                                style={{
                                  backgroundColor: hex,
                              ...
                              >
                                {isSelected && (
                                  <Check className="w-4 h-4 text-brand-gold mix-blend-difference" />
                                )}
                              </button>
                            );
                          })}
                      ...
                      <div className="space-y-3">
                        <div className="flex justify-between items-center">
                          ...
                          <button 
                            ...
                            className="text-[10px] text-brand-gold underline underline-offset-4 font-semibold tracking-wider uppercase cursor-pointer"
                          >
                            Size Guide
                          </button>
                        </div>
                          ...
                          {selectedProduct.sizeOptions.map((size) => {
                            ...
                            return (
                              <button
                                ...
                                className={`px-5 py-3.5 border text-xs tracking-wider uppercase font-medium rounded-sm transition-all duration-300 cursor-pointer ${
                                  isSelected
                                    ? "border-brand-gold bg-brand-gold text-white"
                                    : "border-brand-outline-variant bg-transparent text-brand-outline hover:border-brand-gold"
                                }`}
                              >
                                {size}
                              </button>
                  ...
                  <div className="space-y-6 pt-10">
                    <motion.button
                      ...
                      onClick={() => {
                        const material = detailSelectedMaterial || selectedProduct.materialOptions?.[0] || "#E5D5BC";
                        const size = detailSelectedSize || selectedProduct.sizeOptions?.[0] || "One Size";
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                      {mergedRecentOrders.map((ord) => {
                        ...
                        return (
                          <button
                            key={ord.orderNumber?.toString() || ord.id}
                            type="button"
                            onClick={() => {
